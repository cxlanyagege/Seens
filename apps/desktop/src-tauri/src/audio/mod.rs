...
enum AudioCommand {
    Load { path: String, reply: Sender<Result<LoadedTrack, String>> },
    Play(Sender<CommandResult>),
    Pause(Sender<CommandResult>),
    Stop(Sender<CommandResult>),
    Seek { seconds: f64, reply: Sender<CommandResult> },
    Volume { volume: f32, reply: Sender<CommandResult> },
...
impl AudioPlayer {
    fn request<T>(&self, command: impl FnOnce(Sender<Result<T, String>>) -> AudioCommand) -> Result<T, String> {
        let (reply, response) = mpsc::channel();
        self.commands.send(command(reply)).map_err(|_| "The audio thread is unavailable".to_string())?;
        response.recv().map_err(|_| "The audio thread stopped unexpectedly".to_string())?
    }
}
...
fn audio_worker(receiver: Receiver<AudioCommand>) {
    ...
    while let Ok(command) = receiver.recv() {
        match command {
            AudioCommand::Load { path, reply } => { let _ = reply.send(load(&mut player, path)); }
            AudioCommand::Play(reply) => { let _ = reply.send(with_sink(&player, |sink| {
                if sink.empty() { return Err("The track has finished; select it again to replay".into()); }
                sink.play(); Ok(())
            })); }
            AudioCommand::Pause(reply) => { let _ = reply.send(with_sink(&player, |sink| { sink.pause(); Ok(()) })); }
            AudioCommand::Stop(reply) => {
                if let Some(sink) = player.sink.take() { sink.stop(); }
                player.stream = None; player.path = None; player.duration_seconds = 0.0;
                let _ = reply.send(Ok(()));
            }
            AudioCommand::Seek { seconds, reply } => { let _ = reply.send(with_sink(&player, |sink| {
                let target = seconds.clamp(0.0, player.duration_seconds);
                sink.try_seek(Duration::from_secs_f64(target)).map_err(|error| format!("Could not seek in this track: {error}"))
            })); }
            AudioCommand::Volume { volume, reply } => { let _ = reply.send(with_sink(&player, |sink| { sink.set_volume(volume.clamp(0.0, 1.0)); Ok(()) })); }
            AudioCommand::Status(reply) => { let _ = reply.send(Ok(status(&player))); }
        }
    }
}

fn with_sink<T>(player: &PlayerInner, action: impl FnOnce(&Sink) -> Result<T, String>) -> Result<T, String> {
...
fn load(player: &mut PlayerInner, path: String) -> Result<LoadedTrack, String> {
    ...
    let source = Decoder::try_from(file).map_err(|error| format!("Unsupported or invalid audio file: {error}"))?;
    let duration_seconds = source.total_duration().unwrap_or_default().as_secs_f64();
    let stream = OutputStreamBuilder::open_default_stream().map_err(|error| format!("Could not open the system audio output: {error}"))?;
    let sink = Sink::connect_new(stream.mixer());
    sink.append(source);
    sink.pause();

    if let Some(previous) = player.sink.take() { previous.stop(); }
    player.stream = Some(stream);
    player.sink = Some(sink);
    player.duration_seconds = duration_seconds;
    player.path = Some(path.clone());

    let file_name = source_path.file_stem().and_then(|name| name.to_str()).unwrap_or("Unknown track").to_string();
    Ok(LoadedTrack { path, file_name, duration_seconds })
}
...
fn status(player: &PlayerInner) -> PlayerStatus {
    match player.sink.as_ref() {
        ...
        None => PlayerStatus { loaded: false, playing: false, finished: false, position_seconds: 0.0, duration_seconds: 0.0 },
    }
}
